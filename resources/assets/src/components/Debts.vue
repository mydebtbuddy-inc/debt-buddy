        ...
        <b-card no-body class="mb-4">
          ...
          <b-card-body class="d-flex justify-content-between align-items-start pb-3">
            ...
            <b-dropdown variant="default icon-btn borderless btn-round md-btn-flat hide-arrow" size="sm" :right="!isRTL">
              ...
              <b-dropdown-item href="javascript:void(0)" @click="removeDebt(debt.id)">Remove</b-dropdown-item>
            </b-dropdown>
          </b-card-body>
          ...
          <b-card-body class="pb-3">
            <p class="text-center" v-if="!paymentPlan">
              No payment plan found.<br />
              <router-link to="/payment-plan">Start a debt payment plan</router-link> to see your potential savings.
            </p>
            <p v-else>Paid on the {{ formatBillDay(debt.payment_period) }} of every month</p>
          </b-card-body>
          ...
          <b-card-body class="pt-0">
            ...
            <div class="row mt-3" v-if="paymentPlan">
              <div class="col">
                <div class="text-muted small">Interest Savings</div>
                <div class="font-weight-bold">{{ formatMoney(debt.no_payment_plan.total_interest_paid - debt.payment_plan.total_interest_paid) }}</div>
              </div>
              <div class="col">
                <div class="text-muted small">Remaining Payments</div>
                <div class="font-weight-bold">{{ remainingPayments(debt) }}</div>
              </div>
              <div class="col">
                <div class="text-muted small">Final Payment Date</div>
                <div class="font-weight-bold">{{ debt.payment_plan.billing_dates.slice(-1)[0]['payment_date_formatted'] }}</div>
              </div>
            </div>
          </b-card-body>
        </b-card>
...
export default {
  ...
  methods: {
    ...
    remainingPayments(debt) {
      const today = new Date()

      var remainingPayments = debt.payment_plan.billing_dates.filter((dates) => {
    ...
    removeDebt(debtID) {
      this.$store.dispatch('removeDebt', debtID)
        .then(response => {})
        .catch(error => {
          console.log(error)
        })
